...
class Proqram
{
    ...
    static void Main(string[] args)
    {
        List<Debtor> debtors = new List<Debtor> {
            ...
            new Debtor("Carla A. Creagh", DateTime.Parse("November 21, 1941"), "614-307-8974", "CarlaACreagh@dayrep.com", "3106 Bates Brothers Road Columbus, OH 43215", 11271),
            ...
            new Debtor("James E. Denning", DateTime.Parse("May 4, 1989"), "504-289-8640", "JamesEDenning@jourrapide.com", "1444 Rose Avenue Metairie, LA 70001", 8176),
            ...
            new Debtor("Pamela H. Beauchamp", DateTime.Parse("November 20, 1939"), "801-559-6347", "PamelaHBeauchamp@jourrapide.com", "3239 Tori Lane Salt Lake City, UT 84104", 6588)
        };
        //2
        //var db = debtors.Where(d => d.Email.EndsWith("rhyta.com") || d.Email.EndsWith("dayrep.com")).ToList();
        //db.ForEach(debtors => Console.WriteLine(debtors));
        //3
        //var birthday = debtors.Where(d => d.Birthday.Year<1998 && d.Birthday.Year>1988 ).ToList();
        //birthday.ForEach(debtors => Console.WriteLine(debtors));
        //4
        //var borc = debtors.Where(d => d.Debt<5000).ToList();
        //borc.ForEach(debtors => Console.WriteLine(debtors));
        //5
        //var yas = debtors.Where(n => n.Fullname.Length > 18 || n.Phone.Count(c => c == '7') > 2).ToList();
        //yas.ForEach(debtors => Console.WriteLine(debtors));
        //7
        //var qis = debtors.Where(n => n.Birthday.Month == 12 || n.Birthday.Month == 1 || n.Birthday.Month == 2).ToList();
        //qis.ForEach(debtors => Console.WriteLine(debtors));
        //8
        //var filteredDebtors = debtors.Where(d => d.Debt > debtors.Average(d => d.Debt)).ToList();
        //var sortedDebtors = filteredDebtors.OrderBy(d => d.Fullname).ThenByDescending(d => d.Debt).ToList();
        //sortedDebtors.ForEach(debtors => Console.WriteLine(debtors));
        //9
        //var db = debtors.Where(d=> !d.Phone.ToString().Contains("8")).Select(d => new {Fullname=d.Fullname,Age=2024-d.Birthday.Year,Debt=d.Debt}).ToList();
        //db.ForEach(debtors => Console.WriteLine(debtors));
        //16
        //var db = debtors.Where(d => d.Birthday.Year <= 1945 && d.Birthday.Year>=1939).ToList();
        //db.ForEach(debtors => Console.WriteLine(debtors));
        //15
        //var totalDebt = debtors.Sum(d => d.Debt);
        //Console.WriteLine($"Total Debt: {totalDebt}");
        //14
        //var db = debtors.OrderByDescending(d => d.Debt).Take(5).ToList();
        //db.ForEach(debtors => Console.WriteLine(debtors));
